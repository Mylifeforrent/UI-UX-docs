...
本流程专为**不熟悉 UI 设计的后端/全栈开发者**写。你不需要会画图、懂配色，需要的是把你已经擅长的工程思维迁移过来：

- **你会做的**：定义接口契约、写校验规则、列异常分支、写断言。这套流程里，Prompt = 接口契约，约束 = 校验规则，状态矩阵 = 异常分支穷举，验收标准 = 断言。详见 [Prompt 与上下文](prompting-and-context.md)。
- **你要补的**：信息层级（什么最该显眼）、状态完整性（别只做正常态）、无障碍与令牌化（企业级门槛）。这三点本文每步都会点出。
- **心态**：AI 负责"生成与穷举"，你负责"裁决与取舍"。别把业务权限判断交给 AI（见 [Agent 协作](agent-collaboration.md)）。

## 低保真 vs 高保真

同一条流程能产出两种保真度，按需选择，别混用：

| 维度 | 低保真（线框/灰模） | 高保真（可交互原型） |
| --- | --- | --- |
| 目的 | 快速对齐结构与信息层级 | 验证真实交互、视觉、文案 |
| 产出 | 灰度线框（纯文本即可）| Figma Make code-backed 原型 |
| 提示词重点 | 容器、层级、主次操作位置；**不碰颜色** | 全状态、真实文案、令牌、响应式 |
| 验收 | 结构对不对、有没有漏状态 | 逐状态点一遍、长文案不溢出、危险流程走得通 |
| 对应步骤 | 第 3 步线框 | 第 4 步 Make 生成 |
...
- ❌ 坏：`这几张竞品截图怎么样，给点建议` —— 无角色、无输出结构、会给一堆泛泛套话。
- ✅ 好：上面的 Prompt —— 指定"产品分析师"角色、逐屏结构化输出、强制"结论要指到截图"、明确禁止加新功能。可复现、可裁决。

后端视角：这一步等于"读竞品的接口文档"——你不是抄它的实现，而是看它把哪些字段（操作、状态）暴露在哪个层级。
...
- ❌ 坏：`帮我梳理下业务` —— AI 不知道你的角色/对象/权限，只能瞎猜。
- ✅ 好：上面的 Prompt —— 五列结构 + "权限不足按页面状态设计" + "不新增未提及能力" + "列出待确认问题"。约束把 AI 的臆造挡在门外。

后端视角：五列模型 = 你的领域模型（角色=actor，对象=entity，风险任务=需要事务/审计的写操作）。风险任务的"失败恢复"就是回滚策略，务必人工定夺。
